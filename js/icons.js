/**
 * icons.js - centralised SVG icon library
 * all icons are inline SVG, 16×16 or 20×20 viewBox, stroke-based.
 * usage: CIPHER_ICONS.get('shield') → SVG string
 */
window.CIPHER_ICONS = (function () {
  'use strict';
  const icons = {
    // nav & ui
    moon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z"/></svg>`,
    ...
    plug:     `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18"/><path d="M7 6h1a2 2 0 0 1 2 2v1"/><path d="M14 15v1a2 2 0 0 0 2 2h1"/><path d="M3.05 3.05l17.9 17.9"/><path d="M12 2v2"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="M12 20v2"/></svg>`,
    ...
    certificate:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/><polyline points="9 11 11 13 15 9"/></svg>`,
    signal:   `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="2" y1="20" x2="22" y2="20"/><line x1="5" y1="20" x2="5" y2="14"/><line x1="9" y1="20" x2="9" y2="8"/><line x1="13" y1="20" x2="13" y2="4"/><line x1="17" y1="20" x2="17" y2="10"/><line x1="21" y1="20" x2="21" y2="16"/></svg>`,
    microscope:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18h8"/><path d="M3 22h18"/><path d="M14 22a7 7 0 1 0 0-14h-1"/><path d="M9 14h2"/><path d="M9 12a2 2 0 0 1-2-2V6h6v4a2 2 0 0 1-2 2z"/><path d="M12 6V3a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v3"/></svg>`,
    playCircle:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/></svg>`,
    locate:   `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/><circle cx="12" cy="12" r="8" stroke-dasharray="2 3"/></svg>`,
    // features / perks
  ...
  return {
    ...
    // replace all [icon:name] placeholders in a DOM tree
    hydrate(root) {
      (root || document).querySelectorAll('[data-icon]').forEach(el => {
        const name = el.dataset.icon;
        const size = el.dataset.iconSize || el.dataset.size || 16;
        const svg  = icons[name];
        if (svg) el.innerHTML = svg.replace('<svg ', `<svg width="${size}" height="${size}" `);
      });
    },
  };
})();
